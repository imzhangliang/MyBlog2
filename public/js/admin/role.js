require(['/js/libs/require.config.js'], function(){
    ...
    require(['jquery', 'layer', 'bootstrap','bootstrap-table', 'bootstrap-table-zh-CN'], function($){
        layer.config({
            path: '/js/libs/layer/'
          })

        $(function(){
            var $table = $('#roleAdminTable');
            ...
            $(".editRole").click(function(){
                ...
                if (row) {
                    layer.open({
                        type: 2,
                        content: '/admin/editRole?id=' + row.id,
                        title: '编辑角色',
                        area: ['450px', '80%']
                    })
                }
            ...
            $(".deleteRole").click(function(){
                ...
                if (rows.length == 0) {
                    ...
                    for(let i = 0; i < rows.length; i++) {
                        let item = rows[i];
                        rolenames += item.name + '<br>\n';
                        if (i >= 3) {
                            titles += '...<br>';
                            break;
                        }
                    }
                    let message = `你准备删除以下条目吗:<br> ${rolenames}`;
        
                    layer.confirm(message, {icon: 3}, function(){
                        $.post(`/api/role/deleteRole/${rows[0].id}`, {}, function(data){
            ...
            $table.bootstrapTable({
                url: '/api/role/roleList',        //ajax数据url
                method: 'POST',
